...
В обоих режимах отчёт показывает все 4 уровня формата (`OFF / SIMPLE / PARAM / ACTIVE`) бок о бок, с графом процесса, резидуальной программой, форматами на базисных конфигурациях и сводкой эффекта декомпозиции.
...
- `runner.py` — единое ядро, превращает SLL в `CompileResult` (резидуал, DOT/SVG, форматы, статистика). Используется обоими режимами.
